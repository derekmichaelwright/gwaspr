...
gg_Manhattan_Traits <- function (
  ...
  mp1 <- mp1 +
    ...
    scale_fill_manual(name = NULL, values = alpha(trait.colors, 0.8)) +
  ...
  if(addQQ == T) {
    mp2 <- mp2 +
      geom_point(aes(color = Trait), pch = 1, size = 1.25, alpha = 0.8) +
...
#folder = "GWAS_Results/"; traits = list_Traits(folder)[1:4];
#title = NULL; threshold = 6.8; sug.threshold = 5;
#chrom = NULL;
#markers = myMarkers; labels = markers; vlines = markers;
