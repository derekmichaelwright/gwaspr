...
gg_Manhattan_Zoom1 <- function(
  ...
  fnames <- fnames[grepl(paste0(model, ".", traits, collapse="|"), fnames)]
  fnames <- fnames[grepl(paste0(traits, c(".csv","\\("), collapse="|"), fnames)]
  #
  if(!is.null(skyline)) {
  ...
  for(i in fnames) {
    ...
    xi <- xi %>% mutate(Model = mod, Trait = trt, Type = sky, negLog10_P = -log10(P.value))
    xx <- bind_rows(xx, xi)
  }
  #
  ...
  #
  model <- unique(xx$Model)
  if(paste(model,collapse=",") != "MLM,MLMM,FarmCPU,BLINK,GLM,CMLM,SUPER" &
     paste(model.colors,collapse=",") == "darkgreen,darkred,darkorange3,steelblue,darkorchid4,burlywood4,darkseagreen4") {
    model.colors <- model.colors[c("MLM", "MLMM", "FarmCPU", "BLINK", "GLM", "CMLM", "SUPER") %in% model]
  }
  #
  ...
  #
  if(facet == T) {
    mp <- mp + facet_grid(Trait ~ Chr, scales = "free") +
