...
```{r eval = F, echo = F}
# Order GWAS result files by p value
order_GWAS_Results(folder = "GWAS_Results/")
# Remove all non-GWAS result files (pdf files)
clean_GWAS_Results(folder = "GWAS_Results/") 
```

```{r eval = F, echo = F}
setdiff(myY$Name, t(myG[1,])) # should be character(0)
setdiff(t(myG[1,12:ncol(xG)]), myY$Name) # should show just the first 11 columns of myG
```
