---
title: "gg_manhattan()"
description: "Create Manhattan plots from GAPIT GWAS results."
...
The default setting for `gg_Manhattan()` is to set `facet = F` which will plot all models together. By changing `facet = T` we can separate each GWAS model. When setting `facet = T`, points are now colored by chromosome, which can be changed with `chr.colors`. Significant associations can also be highlighted by setting `highlight.sig` and `sig.color`.
